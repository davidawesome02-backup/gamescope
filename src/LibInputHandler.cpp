...
namespace gamescope
{
    ...
    void CLibInputHandler::OnPollIn()
    {
        ...
        // static int8_t super_key_held = 0; // uint to count number of super keys held (for left and right to not error)
		...
		while ( libinput_event *pEvent = libinput_get_event( m_pLibInput ) )
        {
            ...
            // fprintf( stderr,"Got input: %u, %u\n", (!g_bGrabbed), (g_libinputSelectedDevices.size() > 0));
